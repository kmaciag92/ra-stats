...
NEXT_SHOW_RDS=`cat /stats/get_api_timeslots.json | jq '. | .[] ' | jq -s "sort_by(.weekday,.begin_h,.begin_m) | .[] | select((.weekday*24*60+.begin_h*60+.begin_m >= $(expr $(date +%u) \* 24 \* 60 + $(date +%H) \* 60 + $(date +%M)) and .weekday*24*60+.begin_h*60+.begin_m+.duration > $(expr $(date +%u) \* 24 \* 60 + $(date +%H) \* 60 + $(date +%M)))  or .weekday*24*60+.begin_h*60+.begin_m+.duration >= $(expr $(date +%u) \* 24 \* 60 + $(date +%H) \* 60 + $(date +%M) + 7 \* 60 \* 24)) | .program.rds " | sed 's/\"//g' | head -1`
PREVIOUS_SHOW_RDS=`cat /stats/get_api_timeslots.json | jq '. | .[] ' | jq -s "sort_by(.weekday,.begin_h,.begin_m) | .[] | select((.weekday*24*60+.begin_h*60+.begin_m <= $(expr $(date +%u) \* 24 \* 60 + $(date +%H) \* 60 + $(date +%M))) and .weekday*24*60+.begin_h*60+.begin_m+.duration <= $(expr $(date +%u) \* 24 \* 60 + $(date +%H) \* 60 + $(date +%M)) ) | .program.rds " | sed 's/\"//g' | tail -1`
...
RA_SHOW_ID=`cat /stats/get_api_timeslots.json | jq ". | .[] | select((.weekday*24*60+.begin_h*60+.begin_m <= $(expr $(date +%u) \* 24 \* 60 + $(date +%H) \* 60 + $(date +%M)) and .weekday*24*60+.begin_h*60+.begin_m+.duration > $(expr $(date +%u) \* 24 \* 60 + $(date +%H) \* 60 + $(date +%M)))  or .weekday*24*60+.begin_h*60+.begin_m+.duration >= $(expr $(date +%u) \* 24 \* 60 + $(date +%H) \* 60 + $(date +%M) + 7 \* 60 \* 24)) | .program.slug " | sed 's/\"//g'`
RA_SHOW_REPLAY=`cat /stats/get_api_timeslots.json | jq ". | .[] | select((.weekday*24*60+.begin_h*60+.begin_m <= $(expr $(date +%u) \* 24 \* 60 + $(date +%H) \* 60 + $(date +%M)) and .weekday*24*60+.begin_h*60+.begin_m+.duration > $(expr $(date +%u) \* 24 \* 60 + $(date +%H) \* 60 + $(date +%M)))  or .weekday*24*60+.begin_h*60+.begin_m+.duration >= $(expr $(date +%u) \* 24 \* 60 + $(date +%H) \* 60 + $(date +%M) + 7 \* 60  \* 24)) | .replay " | sed 's/\"//g'`
RA_SHOW_RDS=`cat /stats/get_api_timeslots.json | jq ". | .[] | select((.weekday*24*60+.begin_h*60+.begin_m <= $(expr $(date +%u) \* 24 \* 60 + $(date +%H) \* 60 + $(date +%M)) and .weekday*24*60+.begin_h*60+.begin_m+.duration > $(expr $(date +%u) \* 24 \* 60 + $(date +%H) \* 60 + $(date +%M)))  or .weekday*24*60+.begin_h*60+.begin_m+.duration >= $(expr $(date +%u) \* 24 \* 60 + $(date +%H) \* 60 + $(date +%M) + 7 \* 60 \* 24)) | .program.rds " | sed 's/\"//g'`
...
if [[ "$RA_TAG_COMPRESSED" != *"$RA_SHOW_RDS_COMPRESSED"* ]]; then
  ...
  if [[ "$RA_TAG_COMPRESSED" == *"$PREVIOUS_SHOW_RDS_COMPRESSED"* ]]; then
    RA_SHOW_ID=`cat /stats/get_api_timeslots.json | jq '. | .[] ' | jq -s "sort_by(.weekday,.begin_h,.begin_m) | .[] | select((.weekday*24*60+.begin_h*60+.begin_m <= $(expr $(date +%u) \* 24 \* 60 + $(date +%H) \* 60 + $(date +%M))) and .weekday*24*60+.begin_h*60+.begin_m+.duration <= $(expr $(date +%u) \* 24 \* 60 + $(date +%H) \* 60 + $(date +%M)) ) | .program.slug " | sed 's/\"//g' | tail -1`
    RA_SHOW_REPLAY=`cat /stats/get_api_timeslots.json | jq '. | .[] ' | jq -s "sort_by(.weekday,.begin_h,.begin_m) | .[] | select((.weekday*24*60+.begin_h*60+.begin_m <= $(expr $(date +%u) \* 24 \* 60 + $(date +%H) \* 60 + $(date +%M))) and .weekday*24*60+.begin_h*60+.begin_m+.duration <= $(expr $(date +%u) \* 24 \* 60 + $(date +%H) \* 60 + $(date +%M)) ) | .replay " | sed 's/\"//g' | tail -1`
  fi
fi
...
if [[ "${RA_SHOW_OLD,,}" != "${RA_SHOW_ID,,}" || "${RA_SHOW_ID_AS_PLANNED,,}" != "${RA_SHOW_ID_AS_PLANNED_OLD,,}" ]]; then
  if [[ "${RA_SHOW_OLD,,}" != "playlista" && "${RA_SHOW_ID_AS_PLANNED_OLD,,}" != "playlista" ]]; then
    ...
    SHOW_WEEKDAY=`cat /stats/get_api_timeslots.json | jq ". | .[] | select(.program.slug==\"$RA_SHOW_OLD\") | select(.replay==${RA_SHOW_REPLAY_OLD}) | .weekday"`
